...
// Wallet is an account holding a balance. It exists to demonstrate two
// patterns the Example flow does not use: transactional writes across several
// rows, and cache-aside reads.
type Wallet struct {
...
// balance updates it describes, so a committed transfer always has a record.
